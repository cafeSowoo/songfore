...
async function handleSearch(body, headers) {
  ...
  if (!ttbKey) {
    return jsonResponse({ error: "Missing ALADIN_TTB_KEY" }, 500, headers);
  }
  ...
  if (!title) {
    return jsonResponse({ error: "Missing title" }, 400, headers);
  }
  ...
  try {
    ...
    return jsonResponse({ items: parseAladinXml(xmlText) }, 200, headers);
  } catch (error) {
    return jsonResponse(
